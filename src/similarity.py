...
def symm_KL_sim_matrix(documents, LDA_topic_num = 16, mnitsknn_k = 13, feature_extractor = 'tfidf', verbose=True):
	if feature_extractor = 'tfidf':
		...
		tf_vec = bow_transformer.fit_transform(article_df.content.tolist())
	...
	print('Calculating for {} topics...'.format(n_topics))
	...
	if verbose: print('Theta has been created with dimensions {}'.format(theta.shape))

	if verbose: print('Calculating log_odds from theta matrix... takes a while')
	
	# Symmetric KL Divergence Calculation
	log_odds = np.log(theta[:,None,:]) - np.log(theta[None,:,:])

	if verbose: print('Log odds has been created with dimensions {}'.format(log_odds.shape))

	if verbose: print('Calculating Symmetric KL Divergence values of pairs... Might take a while')
	
	sym_kl = 0.5 * ((theta[:,None,:]*log_odds).sum(-1) - (theta[None,:,:] * log_odds).sum(-1))
	
	if verbose: print('Symmetric KL Divergence values are calculated and dimensions are {}'.format(sym_kl.shape))

	pruned_graph = prune_fully_connected(sym_kl, mnitsknn_k, verbose)
